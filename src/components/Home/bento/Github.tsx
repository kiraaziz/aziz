...
const Github = () => {
  ...
  return (
    <div className={`lg:bg-muted/10 lg:order-1 order-2 lg:border col-span-2 h-full rounded-3xl relative ${!scroll && " lg:p-0"}`}>
      <div className={`w-full  ${scroll ? "animate-pulse opacity-30! h-61" : "h-61"} lg:p-5`}>
        {!scroll && <div className="to-start-animation flex items-center justify-between w-full mb-4">
          ...
          <h1 className="text-lg lg:text-xl font-medium lg:hidden flex ">{metaData.bento.github.titleMobile}</h1>
          <a
            ...
            href={metaData.contact.github.name}
            target="_blank"
            rel="noopener noreferrer"
          >
            <span className="hidden lg:flex">{metaData.bento.github.action}</span>
            <span className="lg:hidden flex ">{metaData.bento.github.actionMobile}</span>
            <ArrowRight size={20} />
          </a>
        </div>}
